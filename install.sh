...
echo ""
echo "  Reword (branch commits only by default):"
echo "    git reflow                            reword branch commits interactively"
echo "    git reflow --claude                   reword via Claude"
echo "    git reflow --openai                   reword via OpenAI"
echo "    git reflow --ollama                   reword via Ollama"
echo "    git reflow HEAD~5 --claude            reword last 5 commits via Claude"
echo ""
echo "  Checkpoint reword (reword only checkpoint commits):"
echo "    git rw ck                             reword checkpoint commits interactively"
echo "    git rw ck --claude                    reword checkpoint commits via Claude"
echo ""
...
echo "    export GIT_REFLOW_ANTHROPIC_API_KEY=sk-ant-...  (for --claude)"
echo "    export GIT_REFLOW_OPENAI_API_KEY=sk-...         (for --openai)"
