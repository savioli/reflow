...
class ClaudeProvider(AIProvider):
    ...
    def __init__(self, api_key: str, model: str, base_url: str | None = None):
        ...
        try:
            ...
            print(
                "Error: anthropic package is required. Install with: pip install anthropic",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    def _call(
        self, prompt: str, schema: dict, tool_name: str, max_tokens: int, system: str
    ) -> dict:
    ...
    def generate(self, prompt: str) -> dict:
        return self._call(
            prompt, COMMIT_SCHEMA, "commit", max_tokens=64, system=self._GENERATE_SYSTEM
        )

    def generate_branch(self, prompt: str) -> dict:
        return self._call(
            prompt, BRANCH_SCHEMA, "branch", max_tokens=32, system=self._BRANCH_SYSTEM
        )
